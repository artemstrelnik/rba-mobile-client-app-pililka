<template>
  <div class="page" :style="`background: #fff;`">
    ...
    <div v-if="!load" class="loyal-login-header-block">
      <div class="loyal-login-header">
        <img :src="require('../../images/pipililka-logo.png').default" />
        <span>PiLiLkA</span>
      </div>
    </div>
    ...
    <div v-if="!load && authStage === 0" class="loyal-login-block">
      <form @submit="checkForm" class="loyal-login-inputs">
        <base-input
          :class="
            phoneInputFocus
              ? 'loyal-login-phone-input loyal-login-phone-input-focused'
              : 'loyal-login-phone-input'
          "
          type="tel"
          pattern="+7 ### ###-##-##"
          name="phone"
          placeholder="Введите номер телефона"
          :onFocusedPhoneInput="onFocusedPhoneInput"
          :onBlurPhoneInput="onBlurPhoneInput"
        />
        <primary-button
          :class="phoneInputFocus ? 'primary-button-focused' : 'primary-button'"
          title="Продолжить"
        />
    ...
    <div v-if="!load && authStage === 2" class="loyal-login-block">
      <form @submit="getUserToken" class="loyal-login-inputs">
        ...
        <i-input
          ...
          name="password"
          :icon="require('../../images/eye-icon-password.svg').default"
        />
...
export default {
  ...
  methods: {
    onFocusedPhoneInput: function () {
      this.phoneInputFocus = true;
    },
    onBlurPhoneInput: function () {
