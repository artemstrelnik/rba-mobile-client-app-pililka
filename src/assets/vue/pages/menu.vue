  ...
  methods: {
    ...
    getSwiperInstance() {
      ...
      this.mySwiperInstance.on('slideNextTransitionEnd', () => {
        const scrolled = this.nowScroll + (this.menu[this.selectedCategory].name.length * 12);
      ...
      this.mySwiperInstance.on('slidePrevTransitionEnd', () => {
        const scrolled = this.nowScroll - (this.menu[this.selectedCategory - 1].name.length * 11 + 12);
        this.selectedCategory--
        document.getElementById('category').scrollTo({
          top: 0,
          left: scrolled,
